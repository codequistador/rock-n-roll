...
test('Renders the full and empty stars correctly', function(assert) {
  ...
  assert.equal(this.$('.glyphicon-star').length, 2, 'The right amount of stars is rendered after changing rating');
  assert.equal(this.$('.glyphicon-star-empty').length, 8, 'The right amount of empthy stars is rendered after changing rating');
